...
class HotelList extends Component{
    ...
    refreshList(){
        axios.get(`${ApiUrl}sales/payment_methods/`,{
    ...
    onDeleteRow = (row) =>{
        if(window.confirm("¿Desea eliminar '"+row.name+"' de hotel?")){
            axios.delete(`${ApiUrl}sales/payment_methods/${row.id}/`)
                .then(res => {
                    this.refreshList();
                });
        }
                ...
                <Table className="no-wrap align-middle" responsive striped bordered id="dataTable">
                    <thead>
                        <tr>
                            ...
                            <th width={"25%"} onClick={(e)=> this.onColumnClick("payment_type__name")}>Tipo de forma de pago {this.sortTable("payment_type__name")}</th>
                            <th width={"25%"} onClick={(e)=> this.onColumnClick("currency")}>Moneda {this.sortTable("currency")}</th>
                            <th width={"13%"}></th>
                        </tr>
                    </thead>
                    <tbody>
                        {data.map((row, index) => (
                            <tr key={row.id}>
                                ...
                                <td className='text-center'>
                                    <ButtonGroup>
                                        <Link to={`/catalogs/payment_method/${row.id}`}>
                                            <Button color="info"
                                                size='sm'>
